...
from diffnext.engine import build_optimizer
...
Diffusion_models = {
    "nova-2B": NOVATransformer3DModel,
    "nova-d48w1024-osp480": NOVATransformer3DModel,  # 0.6B
}

Diffusion_models_class = {
    "nova-2B": NOVATransformer3DModel,
    "nova-d48w1024-osp480": NOVATransformer3DModel,
}
...
def main(args):
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        kwargs = {}
        ae = AutoencoderKLOpenSora.from_pretrained(args.ae_path, cache_dir=args.cache_dir, **kwargs).eval()
        ...
        text_enc_1 = PhiEncoderModel.from_pretrained(args.text_encoder_name_1, **kwargs).eval()
        ...
        if args.pretrained.endswith('.safetensors'):
            ...
            if os.path.exists(os.path.join(args.pretrained, 'config.json')):
                with open(os.path.join(args.pretrained, 'config.json')) as f:
                    config = json.load(f)
                print(f'Load from {args.pretrained} with class_name {NOVATransformer3DModel}')
                load_model = NOVATransformer3DModel.from_pretrained(args.pretrained)
    ...
    # Optimizer creation
    optimizer = build_optimizer(params_to_optimize)
    ...
    def train_one_step(step_, data_item_, prof_=None):
        ...
        if args.extra_save_mem:
            torch.cuda.empty_cache()
            ae.to(accelerator.device, dtype=torch.float32 if args.vae_fp32 else weight_dtype)
            text_enc_1.to(accelerator.device, dtype=weight_dtype)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text_encoder_tokenizer", type=str, default=None)
